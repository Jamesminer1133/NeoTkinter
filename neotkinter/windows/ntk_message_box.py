...
class NTkMessagebox(NTkToplevel):
    ICONS = {
        "check": None,
        "cancel": None,
        "info": None,
    ...
    def __init__(self,
                 ...
                 width: int = 400,
                 height: int = 200,
                 title: str = "NTkMessagebox",
                 message: str = "This is a NTkMessagebox!",
                 option_1: str = "OK",
                 option_2: str = None,
                 option_3: str = None,
                 options: list = [],
        ...
        if bg_color=="default":
            self.bg_color = self._apply_appearance_mode(ThemeManager.theme["NTkFrame"]["fg_color"])
        else:
        ...
        if fg_color=="default":
            self.fg_color = self._apply_appearance_mode(ThemeManager.theme["NTkFrame"]["top_fg_color"])
        else:
            self.fg_color = fg_color
            
        default_button_color = self._apply_appearance_mode(ThemeManager.theme["NTkButton"]["fg_color"])
        ...
        if text_color=="default":
            self.text_color = self._apply_appearance_mode(ThemeManager.theme["NTkLabel"]["text_color"])
        else:
            self.text_color = text_color

        if title_color=="default":
            self.title_color = self._apply_appearance_mode(ThemeManager.theme["NTkLabel"]["text_color"])
        else:
            self.title_color = title_color
            
        if button_text_color=="default":
            self.bt_text_color = self._apply_appearance_mode(ThemeManager.theme["NTkButton"]["text_color"])
        else:
            self.bt_text_color = button_text_color

        if button_hover_color=="default":
            self.bt_hv_color = self._apply_appearance_mode(ThemeManager.theme["NTkButton"]["hover_color"])
        else:
            self.bt_hv_color = button_hover_color
            
        if border_color=="default":
            self.border_color = self._apply_appearance_mode(ThemeManager.theme["NTkFrame"]["border_color"])
        else:
        ...
        self.frame_top = NTkFrame(self, corner_radius=self.round_corners, width=self.width, border_width=self.border_width,
        ...
        if self.cancel_button=="cross":
            self.button_close = NTkButton(self.frame_top, corner_radius=10, width=0, height=0, hover=False, border_width=0,
            ...
            self.button_close = NTkButton(self.frame_top, corner_radius=10, width=10, height=10, hover=False, border_width=0,
        ...
        self.title_label = NTkLabel(self.frame_top, width=1, text=self._title, text_color=self.title_color, font=self.font)
        ...
        self.info = NTkButton(self.frame_top,  width=1, height=self.height/2, corner_radius=0, text=self.message, font=self.font,
        ...
        self.button_1 = NTkButton(self.frame_top, text=self.option_text_1, fg_color=self.button_color[0],
        ...
        if option_2:     
            self.button_2 = NTkButton(self.frame_top, text=self.option_text_2, fg_color=self.button_color[1],
                                                    width=self.button_width, font=self.font, text_color=self.bt_text_color,
                                                    hover_color=self.bt_hv_color, height=self.button_height,
        ...
        if option_3:
            self.button_3 = NTkButton(self.frame_top, text=self.option_text_3, fg_color=self.button_color[2],
                                                    width=self.button_width, font=self.font, text_color=self.bt_text_color,
                                                    hover_color=self.bt_hv_color, height=self.button_height,
    ...
    def load_icon(self, icon, icon_size):
        if icon not in self.ICONS or self.ICONS[icon] is None:
            if icon in ["check", "cancel", "info", "question", "warning"]:
                basePath = Path(__file__).resolve().parents[2]  # adjust depth if needed
                image_path = basePath / "neotkinter" / "assets" / "icons" / f"{icon}.png"
            else:
                image_path = icon
            if icon_size:
            ...
            self.ICONS[icon] = NTkImage(Image.open(image_path), size=size)
