...
from .ntk_frame import NTkBaseClass
from .font import NTkFont
from functools import partial
import tkinter as tk
from typing import Callable

class _NTkOptionButton(NTkButton):
    """
    Internal button class for dropdown menu options.
    
    This class extends NTkButton to provide custom functionality for dropdown menu options,
    including parent menu reference and custom configuration methods.
    """
    
    def setParentMenu(self, menu: "NTkDropdownMenu"):
        """
        Set the parent menu for this option button.
        
        Args:
            menu (NTkDropdownMenu): The parent dropdown menu instance.
        """
...
class _NTkSubmenuButton(_NTkOptionButton):
    """
    Internal button class for dropdown menu submenu options.
    
    This class extends _NTkOptionButton to provide submenu functionality,
    allowing buttons to open child dropdown menus.
    """
    
    def setSubmenu(self, submenu: "NTkDropdownMenu"):
        """
        Set the submenu for this button.
        
        Args:
            submenu (NTkDropdownMenu): The submenu instance to associate with this button.
        """
...
class NTkDropdownMenu(NTkFrame):
    """
    A custom dropdown menu widget for NTkMenuBar.
    ...
    def __init__(self, 
                 widget: NTkBaseClass | _NTkSubmenuButton,
                 master: any = None,
                 border_width: int = 1,
                 width: int = 150,
                 height: int = 25,
                 ...
                 font: NTkFont = ("helvetica", 12),
                 padx: int = 3,
                 pady: int = 3,
                 cursor: str = "hand2",
                 **kwargs):
        """
        Initialize a NTkDropdownMenu instance.
        ...
        Args:
            ...
            **kwargs: Additional arguments passed to parent NTkFrame
        """
        
        if widget.master.winfo_name().startswith("!ntktitlemenu"):
        ...
        elif widget.master.winfo_name().startswith("!ntkmenubar"):
        ...
        self._options_list: list[_NTkOptionButton | _NTkSubmenuButton] = []
    ...
    def add_option(self, option: str, command: Callable=dummy, **kwargs) -> None:
        ...
        Returns:
            _NTkOptionButton: The created option button widget
        """
        optionButton = _NTkOptionButton(
            self,
            width = self.width,
            height = self.height,
            text=option,
    ...
    def add_submenu(self, submenu_name: str, **kwargs) -> "NTkDropdownMenu":
        """
        ...
        Returns:
            NTkDropdownMenu: The created submenu instance
        """
        submenuButtonSeed = _NTkSubmenuButton(self, text=submenu_name, anchor="w",
                                              text_color=self.text_color,
    ...
    def remove_option(self, option_name: str) -> bool:
        ...
        for i, option in enumerate(self._options_list):
            if option.cget("text") == option_name:
                ...
                if isinstance(option, _NTkSubmenuButton):
    ...
    def _show(self, *args, **kwargs) -> None:
        ...
        if isinstance(self.menu_seed_object, _NTkSubmenuButton):
            self.place(
    ...
    def _hideChildrenMenus(self, *args, **kwargs) -> None:
        ...
        Args:
            ...
            **kwargs: Arbitrary keyword arguments
        """
        if any(isinstance(option, _NTkSubmenuButton) for option in self._options_list):
            for option in self._options_list:
                if isinstance(option, _NTkSubmenuButton):
    ...
    def _collapseSiblingSubmenus(self, button: _NTkOptionButton | _NTkSubmenuButton, *args, **kwargs) -> None:
        """
        ...
        """
        for option in self._options_list:
            if option != button and isinstance(option, _NTkSubmenuButton):
    ...
    def toggleShow(self, *args, **kwargs) -> None:
        ...
        if widget_base.startswith("!ntktitlemenu") or widget_base.startswith("!ntkmenubar"):
    ...
    def _configureButton(self, button: NTkButton) -> None:
        """
        Configure styling and behavior for a menu button.
        
        Args:
            button (NTkButton): The button to configure
        """
    ...
    def _getSubMenus(self) -> list["NTkDropdownMenu"]:
        """
        Get a list of all submenus associated with this menu.
        
        Returns:
            list[NTkDropdownMenu]: List of submenu instances, empty list if none exist
        """
        if any(isinstance(option, _NTkSubmenuButton) for option in self._options_list):
            subMenusList = list()
            for option in self._options_list: 
    ...
    def _checkIfMouseLeft(self, event: tk.Event=None) -> None:
        ...
        if not self._get_coordinates(event.x_root, event.y_root):
            if isinstance(self.menu_seed_object, _NTkSubmenuButton) and not self.menu_seed_object.parent_menu._get_coordinates(event.x_root, event.y_root):
            ...
            elif not isinstance(self.menu_seed_object, _NTkSubmenuButton):
                subMenus = self._getSubMenus()
                if subMenus == [] or all((not submenu._get_coordinates(event.x_root, event.y_root)) for submenu in subMenus):
                    self._hideAllMenus()
    ...
    def configure(self, **kwargs):
        ...
        for widget in self.winfo_children():
            if (type(widget) is _NTkOptionButton) or (type(widget) is _NTkSubmenuButton):
