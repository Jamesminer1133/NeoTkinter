...
from ..appearance_mode import NTkAppearanceModeBaseClass
from ..scaling import NTkScalingBaseClass

from ..utility import pop_from_dict_by_set, check_kwargs_empty


class NTkBaseClass(tkinter.Frame, NTkAppearanceModeBaseClass, NTkScalingBaseClass):
    """ Base class of every NTk widget, handles the dimensions, bg_color,
        appearance_mode changes, scaling, bg changes of master if master is not a NTk widget """
    ...
    def __init__(self,
        ...
        NTkAppearanceModeBaseClass.__init__(self)
        NTkScalingBaseClass.__init__(self, scaling_type="widget")
        ...
        # overwrite configure methods of master when master is tkinter widget, so that bg changes get applied on child NTk widget as well
        if isinstance(self.master, (tkinter.Tk, tkinter.Toplevel, tkinter.Frame, tkinter.LabelFrame, ttk.Frame, ttk.LabelFrame, ttk.Notebook)) and not isinstance(self.master, (NTkBaseClass, NTkAppearanceModeBaseClass)):
    ...
    def destroy(self):
        ...
        NTkAppearanceModeBaseClass.destroy(self)
        NTkScalingBaseClass.destroy(self)
    ...
    def config(self, *args, **kwargs):
        raise AttributeError("'config' is not implemented for NTk widgets. For consistency, always use 'configure' instead.")
    ...
    def _check_image_type(self, image: any):
        ...
        if image is None:
            return image
        elif isinstance(image, NTkImage):
            return image
        else:
            warnings.warn(f"{type(self).__name__} Warning: Given image is not NTkImage but {type(image)}. Image can not be scaled on HighDPI displays, use NTkImage instead.\n")
            return image
    ...
    def _detect_color_of_master(self, master_widget=None) -> Union[str, Tuple[str, str]]:
        ...
        if isinstance(master_widget, (windows.widgets.core_widget_classes.NTkBaseClass, windows.NTk, windows.NTkToplevel, windows.widgets.ntk_scrollable_frame.NTkScrollableFrame)):
            if master_widget.cget("fg_color") is not None and master_widget.cget("fg_color") != "transparent":
                return master_widget.cget("fg_color")

            elif isinstance(master_widget, windows.widgets.ntk_scrollable_frame.NTkScrollableFrame):
