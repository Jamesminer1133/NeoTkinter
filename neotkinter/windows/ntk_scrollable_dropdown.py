'''
Advanced Scrollable Dropdown class for neotkinter widgets
Author: Akash Bora
'''
...
class NTkScrollableDropdown(NTkToplevel):
    ...
    def __init__(self, attach, x=None, y=None, button_color=None, height: int = 200, width: int = None,
        ...
        self.disable = False
        self.fg_color = ThemeManager.theme["NTkFrame"]["fg_color"] if fg_color is None else fg_color
        self.scroll_button_color = ThemeManager.theme["NTkScrollbar"]["button_color"] if scrollbar_button_color is None else scrollbar_button_color
        self.scroll_hover_color = ThemeManager.theme["NTkScrollbar"]["button_hover_color"] if scrollbar_button_hover_color is None else scrollbar_button_hover_color
        self.frame_border_color = ThemeManager.theme["NTkFrame"]["border_color"] if frame_border_color is None else frame_border_color
        self.button_color = ThemeManager.theme["NTkFrame"]["top_fg_color"] if button_color is None else button_color
        self.text_color = ThemeManager.theme["NTkLabel"]["text_color"] if text_color is None else text_color
        self.hover_color = ThemeManager.theme["NTkButton"]["hover_color"] if hover_color is None else hover_color
        ...
        self.frame = NTkScrollableFrame(self, bg_color=self.transparent_color, fg_color=self.fg_color,
        ...
        self.dummy_entry = NTkEntry(self.frame, fg_color="transparent", border_width=0, height=1, width=1)
        self.no_match = NTkLabel(self.frame, text="No Match")
        self.height = height
        ...
        # Add binding for different ntk widgets
        if double_click or type(self.attach) is NTkEntry or type(self.attach) is NTkComboBox:
            self.attach.bind('<Double-Button-1>', lambda e: self._iconify(), add="+")
        else:
            self.attach.bind('<Button-1>', lambda e: self._iconify(), add="+")

        if type(self.attach) is NTkComboBox:
        ...
        if type(self.attach) is NTkOptionMenu:
    ...
    def bind_autocomplete(self, ):
        ...
        if type(self.attach) is NTkEntry:
    ...
    def _init_buttons(self, **button_kwargs):
        ...
        for row in self.values:
            self.widgets[self.i] = NTkButton(self.frame,
                                                          text=row,
